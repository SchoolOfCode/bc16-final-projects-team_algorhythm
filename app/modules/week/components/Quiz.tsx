...
export default function Quiz({ props }: any) {
  ...
  const dayQuestions = props.quizzes.data.filter(
    (obj: any) => obj.day_number === props.selected,
  );
  ...
  const random = (item: any, index: any) => {
    ...
    const labels = [
      { text: item.correct_answer, id: 1 },
      { text: item.incorrect_answer1, id: 2 },
      { text: item.incorrect_answer2, id: 3 },
      { text: item.incorrect_answer3, id: 4 },
    ];
    // Generate a unique seed based on the item data, index, and current timestamp
    const seed =
      item.correct_answer.length +
      item.incorrect_answer1.length +
      item.incorrect_answer2.length +
      item.incorrect_answer3.length +
      index;
    ...
    return (
      <>
        {shuffledLabels.map((label: any, idx: any) => (
          <label
            key={idx}
            className="bg-socskyblue p-4 mb-2 rounded-md dark:text-black"
            htmlFor={`question${index}${label.id}`}
          >
            {label.text}
            <input
              className="ml-2 radio radio-xs bg-white border-loginblue dark:focus:bg-loginblue"
              type="radio"
              id={`question${index}${label.id}`}
    ...
    <div className="flex flex-col items-center gap-5 mt-2 md:mb-10 md:-mt-10">
      {!submitted ? (
        <>
            ...
            {dayQuestions.length > 0 ? (
                ...
                >
                  ...
                  <div className="join grid grid-cols-2 py-5 px-20 mt-5">
                    {index ? (
                      <a
                        href={`#slide${index - 1}`}
                        className="join-item btn btn-ghost bg-loginblue"
                      >
                        Previous
                      </a>
                    ) : (
                      <a
                        href={`#slide${index - 1}`}
                        className="join-item btn btn-ghost text-black bg-gray-300 pointer-events-none"
                      >
                        Previous
                      </a>
                    )}
                    {index === total - 1 && selected[index] ? (
                      ...
                      <div
                        ...
                        data-tip="Select an answer"
                      >
                        <a
                          href={`#slide${index + 1}`}
                          className="join-item btn btn-ghost w-full bg-gray-300 text-black pointer-events-none"
                        >
                          Next
                        </a>
                      </div>
              ...
              <div className="text-center mt-20 mb-10">
                ...
                <p className="text-md text-gray-600 dark:text-white">
                  We're working hard to bring you new content. Please check back
                  later.
                </p>
              </div>
            )}
          ...
          <div>
            <p
              className="btn btn-ghost bg-loginblue"
              onClick={() => {
